...
- `dh5browser`: fixed `AssertionError` in ephyviewer's navigation toolbar when opening files without continuous data. Spike sources without channels are no longer turned into empty viewers, and sources spanning zero time (empty, or holding a single spike/event) no longer abort the browser.
